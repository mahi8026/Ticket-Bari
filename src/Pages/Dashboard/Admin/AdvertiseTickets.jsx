...
const AdvertiseTickets = () => {
  ...
  const advertisedCount = useMemo(() => {
    return tickets.filter((t) => t.isAdvertised).length;
  }, [tickets]);
  ...
  if (isLoading) {
    return <LoadingSpinner type="page" message="Loading tickets..." />;
  }
    ...
    <div className="p-8">
      ...
      <div className="alert shadow-lg mb-6 bg-info text-white">
        <div>
          ...
          <span>
            **Advertising Limit:** You have **{advertisedCount} out of 6** slots
            used for homepage advertisement. Only Admin-approved tickets are
            shown below.
          </span>
        </div>
      </div>
        ...
        <table className="table w-full">
          ...
          <tbody>
            {tickets.map((ticket, index) => (
              <tr
                key={ticket._id}
                className={ticket.isAdvertised ? "bg-green-50" : ""}
              >
                <th>{index + 1}</th>
                <td>{ticket.title}</td>
                ...
                <td>
                  <input
                    type="checkbox"
                    className="toggle bg-sky-400 toggle-success"
